...
| P1 — паритет метрик и контрактов | 33 | **29** | 4 |
| P2 — полнота DSL, воронки, авторинг | 24 | **1** | 23 |
...
| ~~P1-15~~ | ~~Sticky CTA не заавторен и скрывается под FTC~~ — **СДЕЛАНО 2026-09-09** | S | S |
...
| ~~P1-18~~ | ~~PayPal без гейта `show_paypal_button` и без проверки валют~~ — **СДЕЛАНО 2026-09-09** | S | S |
...
| ~~P1-28~~ | ~~`CLAUDE.md` отсутствует~~ — **СДЕЛАНО 2026-09-09** (SKILL с автоподгрузкой не делал: команда `/new-funnel` уже несёт правила, второй источник разошёлся бы с первым) | AU | S |
| ~~P1-29~~ | ~~`pnpm validate` не входит в `pnpm build`~~ — **СДЕЛАНО 2026-09-09** попутно с P0-19 | AU | S |
| ~~P1-30~~ | ~~Product id не проверяются по живому каталогу~~ — **СДЕЛАНО 2026-09-09**: `pnpm validate:live` | AU | M |
...
| ~~P2-16~~ | ~~Chain не локализован~~ — **СДЕЛАНО 2026-09-09**; попутно найдено, что покрытие локали считалось без цепочки | PP | S |
...
**Хвост P1-12, P1-28, P1-30. — СДЕЛАНО 2026-09-09.**

- **`revenue_started_checkout` нёс шесть отобранных полей.** Прод расплёскивает
  в это событие весь объект подписки (`sendCheckoutEvents.ts:72`) и выводит
  `weekPrice` из `priceDivider` плана, так что дашборд, сегментирующий старты
  чекаута по длине триала, периоду или цене за неделю, читает их прямо с
  события. `condition_id` уходит **присутствующим и null**: прод всегда шлёт
  ключ, а присутствующий null говорит «условия нет» там, где отсутствие ключа
  сказало бы «этот продюсер про условия не знает». `checkout_name` и `cluster`
  остаются отсутствующими, как у прода без CMS — они называют вариант чекаута и
  кластер эксперимента, которые есть только у Strapi-воронок.
- **Visibility-пропсы старта разделены по тому, кто их вообще может увидеть.**
  Прод сэмплит четыре в момент отправки; движок эмитит
  `funnels_started_funnel` на сервере на входном хите, когда документа ещё нет.
  `ua_fb_flags` сервер читает не хуже браузера (in-app WebView Facebook
  объявляет себя в User-Agent, и именно он прогревает воронки, на которые никто
  не смотрел) — уезжает со стартовым событием. `visibility_state`, `has_focus`
  и `time_to_event_ms` уезжают с **подтверждением**: это событие и существует,
  чтобы отделить прогретый WebView от живого визитёра, и оно единственное в
  паре, чьи данные приходят из браузера.
- **`CLAUDE.md`** — 75 строк того, что здесь уже было сделано неправильно:
  ни цен, ни названий компаний в копирайте; ни одной legal-ссылки с этого
  хоста; Funnel Core keys — контракт; один продукт на оффер; комплаенс-копия не
  редакционная. Плюс главное для генерируемой воронки: **если DSL не выражает
  бриф — сказать об этом**, а не приближать бриф к тому, что схема случайно
  разрешает. Названный пробел исправляется один раз для всех воронок, обходной
  путь копируется.
  SKILL с автоподгрузкой не делал сознательно: правила уже несёт команда
  `/new-funnel`, а второй источник тех же правил разошёлся бы с первым.
- **`pnpm validate:live`** проверяет каждый product id, который воронка может
  списать (пейволл, down-sale, все хопы), по `/v1/billing/products`. Это
  единственная проверка, которая отличает выдуманный id от настоящего:
  остальные доказывают, что конфиг корректен, а корректный конфиг с выдуманным
  id — это пейволл, который принимает клики и с которого нельзя купить.
  Неизвестный id — FAIL с указанием, где искать (Walhalla MCP `billing_plans`);
  недоступный каталог — WARN, потому что валидатор, падающий на плохой сети, —
  это валидатор, который начинают пропускать.
  Проверено в обе стороны: живая воронка проходит, а подмена одного хопа на
  `promova_does_not_exist_zzz` роняет прогон с названным id.

Приёмка: 602 кейса, 14/14 сьютов. Коммиты `08ccd3f`, `4045321`.

**P1-31 (таймер: WARN или FAIL) оставлен открытым** — это вопрос Q9 к продукту,
и решать его здесь значило бы решать за них.

**P1-15, P1-18. — СДЕЛАНО 2026-09-09.**

- **Sticky CTA скрывался под FTC по моему рассуждению, а не по чьему-то
  правилу.** В комментарии стояло, что постоянно видимая кнопка покупки — это
  «ровно то давление, которое убирает комплаенс-вариант». Это писал я. Прод
  показывает свой sticky CTA и под FTC; скрывают его только авторский
  выключатель, попадание основного CTA в вьюпорт и открытая форма чекаута
  (`sales-page/Header/Header.tsx:30-33`). Правила FTC — про раскрытие и равную
  заметность отказа, а не про удаление кнопки, и через легал проходил
  прод-вариант. Заодно бар теперь заавторен в живой воронке, где его не было.
  Рендерится `hidden`, показывает его IntersectionObserver, только пока
  настоящий CTA за экраном — прод-поведение, и визитёр без JS видит один CTA,
  а не два друг над другом.
- **PayPal гейтится дважды, как у прода.** `show_paypal_button` — это
  платформенный kill switch с дефолтом true, и без его чтения движок был бы
  единственной поверхностью чекаута, которая продолжает предлагать PayPal
  после выключения везде. И валюта проверяется **до** запроса: PayPal
  сеттлит 23 валюты, так что план в UAH или INR давал провалившийся вызов
  заказа и строку `paypal order failed`, которая ничего не значила. Отказ
  заранее делает остальные ошибки читаемыми: теперь эта строка означает, что
  что-то действительно не так.
- Оба переключателя берутся из одной оценки флагов, общей с чтением sandbox, а
  не из двух запросов по ~100 КБ.

Приёмка: 594 кейса, 14/14 сьютов. Коммит `826adc5`.
...
**P2-16. Локализация цепочки. — СДЕЛАНО 2026-09-09.**
Цепочка рендерилась из авторского конфига: украинский визитёр шёл по
украинскому квизу, читал украинский пейволл, платил — и попадал на английский
апселл. Это худшее место в воронке для смены языка: сразу после списания, пока
человек ещё решает, тому ли сайту он доверился. Механизм тот же, что у квиза и
пейволла (один каталог на воронку на всё дерево), так что строка цепочки
переводится в тот день, когда попадает в `locales/<locale>.json`.

**Попутно: число покрытия локали измеряло две трети воронки.** `validate`
считал его только по квизу и пейволлу, поэтому воронка показывала «97 %
переведено (10 не хватает из 309)», а все 22 непереведённые строки цепочки не
считались вовсе. Теперь 90 % из 331 — честная цифра, и строки видны
`/translate-page`, откуда копия и должна приходить, а не от меня.

Приёмка: 606 кейсов, 14/14 сьютов. Живьём засеял одну строку
(`You're in` → `Ви в грі`): `/alpha/.../thank-you` отдаёт английский,
`/uk/alpha/...` — украинский, непереведённая строка падает на английский, а не
на пустоту. Коммит `fce07fa`.
