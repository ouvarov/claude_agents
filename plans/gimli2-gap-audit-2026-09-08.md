...
| Аналитика | **PARTIAL, с дефектами** | Квиз в паритете и лучше прода (дедуп, серверная отправка). Money-path: события в Amplitude уходят **дважды** без ключа дедупа, серверная отправка **не читает consent** в EEA, апсел шлёт Meta `Purchase` вместо `Upsell`, Google Ads конверсия и backend-CAPI-feed не портированы, `gen_joined_ab_test` не эмитится вовсе. |
...
| Email-шаг + existing user | **DONE с оговорками** | Валидация, Firebase, Reteno, consent — паритет. Дыры: нет rate limit на `/answer`, смена адреса посреди прогона переписывает email аккаунта и удваивает конверсию, сырой email утекает в localStorage платформы, после покупки визитёр приходит на `/complete-sign-up` **без сессии** и уходит на sign-in. |
...
| P0-5 | Money-события в Amplitude уходят дважды (сервер без `insert_id` + зеркало без `eventId`) | A | M |
| P0-6 | Серверный Amplitude/TikTok на money-path игнорирует consent (EEA) | A | S |
| P0-7 | Meta получает `Purchase` на апселе вместо `trackCustom('Upsell')` | A | S |
| P0-8 | Квизовый серверный TikTok шлёт весь конверт с сырым email (PII) | A | S |
...
| P0-14 | Смена email посреди прогона молча переписывает адрес аккаунта и удваивает CompleteRegistration | E | S |
| P0-15 | Сырой email попадает в `localStorage['funnel_builder_user_answers']` и в `selected_options` | E | S |
| P0-16 | Нет rate limit на `/kilo/:id/answer` → enumeration email с IP пода, 429 невидим | E | M |
| P0-17 | `ack`/`done` псевдо-ответы протекают в `/v1/quiz-results` (8 мусорных записей на GE) | O | S |
| P0-18 | Лоадер не ждёт `/prepare`; `qrid` теряется → заказ без `quiz_result_id`; `forced` захардкожен | O | M |
...
**P0-5. Дедуп money-событий в Amplitude.**
...
**P0-14. Смена email посреди прогона.**
