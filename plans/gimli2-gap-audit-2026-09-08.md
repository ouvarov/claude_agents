...
| P2 — полнота DSL, воронки, авторинг | 27 | **3** | 24 |
...
| P1-19 | BLIK / UPI / Pix Automatico / MB Way не подключены; `tokenization_for_wallets_split` не читается. **Переоценён 2026-09-10 с M на S.** Главная неизвестность снята: во фронте прода НЕТ ни редиректов, ни поллинга, ни вебхуков (grep по `poll|return_url|redirect_url|webhook` в `packages/features/Checkout` пустой) — все четыре способа это кнопки внутри того же iframe Solidgate, завершение теми же событиями `success`/`fail`, что уже обрабатываются. Работа = 4 гейта (флаг + страна/валюта) + 4 параметра SDK + 2 поля в теле (`blik: true`, `pix_automatico: true`, `phone` для MB Way). Гонку прода с отложенным монтажом BLIK до резолва гео (`CheckoutForm.tsx:150-163`) движок не наследует — гео известно до рендера. Отдельно и реально: `entity` на успехе → `payment_type: 'rebill'` в апселле под сплитом (см. P3-9) | S/PP | S |
...
| P1-22 | **ЧАСТИЧНО 2026-09-09**: лейблы локализуются там, где есть ревьюленный перевод. **Разблокирован 2026-09-10:** Q17 отвечен — «Legal Info» ведёт на `/contact-us`, нужен роут + ссылка первым элементом. План — `plans/funnel-engine-route-parity.md` §6.1 | L | S |
...
| P2-2 | Типы экранов: swym `results` (4 варианта), swym `profile`, лендинг. **Переформулировано 2026-09-10 решением Алекса:** CMS-билдер лендингов (`LandingBuilder`, 39 блоков в Strapi) **не переносим** — отдельная тема. Движок должен уметь собрать лендинг **сам, своим DSL**, как это умеет Gimli на проде: плоская страница из секций, email-шаг, чекаут. То есть работа не «перенести 39 блоков», а «дать движку тип страницы landing поверх имеющихся 16 секций». Постановка на ресёрч — `plans/funnel-engine-landing-research.md` (2026-09-10); упирается в P2-11 (где живёт контент) и в конфликт со сплошным `noindex` движка | O | M |
| P2-3 | Дыры внутри типов: `reaction`, `steps[]` лоадера, `reassurance`, `bars.pct`, `count(answers)`, `{n}`, lowercase гейта | O | M |
| ~~P2-4~~ | ~~Прогресс-бар: знаменатель = длина flow~~ — **СДЕЛАНО 2026-09-09**: прогресс и `step_number` считают пройденный путь визитёра (`state.visited`), знаменатель — `progressTotal ?? flow.length`, с клампом | O | S |
...
| P2-14 | Timer: хром countdown не рендерится; swap продуктов не гейтится по FTC. **Подтверждено 2026-09-10:** на проде таймер живой и показывается не-FTC визитёрам, так что это перенос, а не удаление. Подавление под FTC у движка уже правильное | S | S |
| P2-26 | Полный паритет по оплате: BLIK, UPI, Pix Automatico, MB Way, `entity` → `payment_type: rebill`, CheckoutDownsell, купоны. План с этапами и списком того, что надо снять отдельным ресёрчем — `plans/funnel-engine-payment-parity.md` (2026-09-10). Пересекается с P1-19 (способы оплаты) и P2-15 (downsell, купоны), но собран в один документ, потому что делается одним заходом | S | S+ |
| P2-27 | Паритет по роутам. **Сужен решениями Алекса 2026-09-10** (`/o`, `/foxtrot`, `/confirmation-banner`, `/profile/*` не нужны — это фаннел-билдер; `/whiskey`, `/xray`, `/romeo` не нужны — код-воронки ходят типами экранов внутри `/kilo`). **СДЕЛАНО в тот же день:** `/echo` (вход + восстановление на своём хосте, закрыло утечку трафика на `promova.com`), `/robots.txt` с доменным сплитом и строками AdsBot. **Оказались уже сделанными:** мидлвара локали и локаль на легале. **Осталось:** `/contact-us` (на проде это Strapi-страница с формой — развилка) и фолбэк неизвестного слага (у прода это заавторенная страница `default`, у движка её нет). Полностью — `plans/funnel-engine-route-parity.md` §8 | O | S |
| P2-25 | FTC-диффы, найденные сверкой 2026-09-10: под FTC движок **убирает** ленту на карточке, а прод **заменяет** её текстом (`PlansWithTimer/.../Card.tsx:109`); в hard-варианте у прода есть зачёркнутая regular-цена (`HardRow`/`HardPricesBlock`), у движка только строка now/then. Остальные шесть ветвлений совпадают дословно: `showPerPeriod`, CTA под FTC, снятие renewal-строки, снятие соц-баннера, hard-only «No payment on this step», one-click отключён под FTC | S | S |
...
| P2-22 | `?lang=` оверрайд; soft-404 на неизвестном `?screen`; семантика resume (новая вкладка, 4 ч). **Уточнено 2026-09-10:** прод на `/sierra`, `/golf`, `/delta`, `/alpha`, `/echo` слаг вообще не проверяет — неизвестный падает на `DEFAULT_SALES_PAGE_SLUG` (`SalesPage.tsx:11,24`), `notFound()` есть только у семейства онбординга. То есть «как на проде» = дефолтная страница, а не 404 | O | S |
| ~~P2-23~~ | ~~Brazil/EBANX дисклоуз~~ — **СДЕЛАНО 2026-09-10**: текст дословно из `CheckoutFooter.tsx:65-81`, включая клаузу про мультикарты, в позиции прода (после ноты о $1, перед строкой CMS), гейт по стране с `null` = не показывать | L | S |
| ~~P2-24~~ | ~~Email-ошибки не локализуются; name-input 40 vs 64, без серверной обрезки~~ — **СДЕЛАНО 2026-09-09**: шесть строк email-шага в `TRANSLATABLE_FIELDS`, имя режется на сервере до 64 code point, `maxlength` 40→64 | E | S |
...
| ~~Q3~~ | ~~Идемпотентность `POST /v3/billing/upsales`~~ — **СНЯТ 2026-09-10** ресёрчем прода. Идемпотентности нет и передать её некуда: `header?: never` в контракте (`schema-payments.d.ts:3603`), в запросе только `Content-Type` (`billing.ts:66-69`). Вся защита прода — инстансный `isProcessingRef` + `disabled` (`Upsell.tsx:204, 665-669`); `checkout_order_id` не удаляется никогда, так что перезагрузка или Back на тот же хоп **списывают второй раз**. У движка тот случай закрыт серверным `state.done` (`post-purchase.ts:628-641`) — мы на паритете или лучше на каждой оси, свой гвард не добавляем. `quiz_result_id: 'empty'` — прод-значение дословно (`Upsell.tsx:239, 754`, хардкод в `UpsellDownsell.tsx:411`). Общая с продом дырка «чардж прошёл, ответ потерян» унаследована осознанно | — | — |
...
| Q7 | Лимит `check_email_provider` по IP: весь трафик движка идёт с адресов пода. **Сужен 2026-09-10 и больше не блокирует P0-16**: прод зовёт эндпоинт только из браузера, без авторизации, без капчи и без throttle (`checkEmailProvider.ts:8-21`, `OpenApiClient.ts:150-153`), и раскрывает визитёру ещё и список провайдеров. У движка вызов серверный, зато с токен-бакетом (`ratelimit.ts`) и корректной деградацией: на 429 падаем в линковку, авторитет — Firebase (`quiz.ts:1038-1042`). Осталось узнать только, есть ли лимит по IP — если есть, нужен аллоулист пода либо форвардед-IP | бэкенд auth | ничего не блокирует |
...
| ~~Q9~~ | ~~Таймер: WARN или FAIL~~ — **ОТВЕЧЕН 2026-09-10 ресёрчем прода, и ответ обратный тому, что предполагал аудит.** Прод countdown НЕ вырезал: живых движка два (`useFunnelBuilderTimer.tsx:26-131` для баннера даун-сейла и `usePersistentCountdown` через `TimerSalesProvider`), оба до сих пор показываются обычным визитёрам и лишь не монтируются под FTC (`Sales.tsx:1975`, `SalesTimerScope.tsx:41-45`, PRMV-17964). Значит FAIL делать нельзя, WARN остаётся, но его текст («cut from swym-2 post-launch») врёт и должен быть переписан. P1-31 закрывается правкой сообщения; P2-14 — настоящая работа | — | — |
| ~~Q10~~ | ~~Локали пилота~~ — **СНЯТ 2026-09-10 решением Алекса:** локали добавим позже, воронки для прода пока нет | — | — |
| ~~Q11~~ | ~~english-hub `landing` — экран или sales-страница?~~ — **ПЕРЕФОРМУЛИРОВАН И ВЫНЕСЕН 2026-09-10.** Ресёрч показал два независимых механизма: (А) `LandingBuilder` — контент целиком в Strapi (`builders`/`ai-landings`), dynamic zone из **39 типов блоков** с концептами (у Hero 12, у WhatYouGet 11), реестр `[__component][concept]`, новый лендинг = работа в CMS без деплоя, A/B через `builder-split-tests` + флаг; (Б) `landing` у english-hub — ручной TS-тип на 30 полей в `code-funnels/english-hub/types.ts:18-83`, захардкожен по локалям, внутри флоу `[landing, email]`, без цен. «Как делают сейчас» — это (А). **Решение Алекса 2026-09-10: LandingBuilder — отдельная тема, не в этом объёме.** Упирается в P2-11 (публикация без пересборки образа) | — | отдельный проект |
...
| ~~Q14~~ | ~~`funnel_name` пилота~~ — **СНЯТ 2026-09-10:** пилота ещё нет, текущая воронка — замер скорости. Причина вопроса тоже ушла: P1-1…P1-3 закрыты, форма данных квиза совпадает с продом | — | — |
| ~~Q15~~ | ~~Бриф на первую воронку~~ — **СНЯТ 2026-09-10:** пилота ещё нет, спрашивать нечего. Вернуть, когда появится воронка для прода | — | — |
...
| ~~Q17~~ | ~~Нужна ли страница «Legal Info»?~~ — **ОТВЕЧЕН 2026-09-10 ресёрчем:** «Legal Info» это не легал-страница, а первый элемент ряда ссылок, ведущий на `/contact-us` (`LegalInfo.tsx:49-157`); у chameleon такой роут есть. Значит нужна страница `/contact-us` на движке и ссылка первым элементом — работа, а не вопрос. Закрывает остаток P1-22. Про `rel=canonical`: у прода на легал-страницах его в коде нет вовсе, только `noindex` на `refund`, так что и у нас не нужен | — | — |

### 4.1 Что ещё ждёт людей, помимо таблицы (сведено 2026-09-09)

**Решения (без них пункт не закрыть):**

| # | что решить | кому | блокирует |
|---|---|---|---|
| ~~Q18~~ | ~~Какие рынки нужны для BLIK/UPI/Pix?~~ — **СНЯТ 2026-09-10 ресёрчем:** рынки задаются не кодом, а флагами GrowthBook (`isShowBlikButton` + PL/PLN, `use_upi_payment` + IN/INR, `isShowPIXButton` + BRL, `isShowMBWayButton` + PT/EUR). Движок читает те же флаги — маркетинг включает страны без нашего участия. GCash в коде прода отсутствует полностью, значит и у нас не делаем | — | — |
| Q19 | `promova-ai-academy.com`: legal-страницы отдают PALM, а `legal-rule` для того же домена называет Promova Limited. Одно из двух — неверные данные в CMS, не код | legal / контент | — (дефект прода) |

**Действия (код готов, ждёт заливки):**

1. **Vault, 26 переменных, свой путь на каждый сабдомен** — лист в
   `plans/funnel-engine-vault-env.md`, там же runbook «добавляем сабдомен» из
   4 шагов. До заливки движок поднимается на дефолтах и не имеет пикселей.
2. **`FIREBASE_SERVICE_ACCOUNT`** (Q2) — переменная заведена и пустая;
   без неё hand-off идёт вторым уровнем (`link_for_auth`).
3. **DevOps-лист** — `plans/funnel-engine-devops-parity.md`: гео-заголовок
   Cloudflare, CDN-правило на `/img/*` и `/vendor/*`, forwarded headers (теперь
   несут нагрузку — `DOMAIN_NAME` и гео), `HOSTNAME`, пробы, `GIT_HASH`.
4. ~~Figma-токен~~ — **снято 2026-09-10**: `FIGMA_ACCESS_TOKEN` не читал
   никто (MCP берёт `FIGMA_API_KEY` из `.mcp.json`, а тот в `.gitignore`),
   так что блок удалён из трёх скриптов, а не переехал в `.env.local`.
   Сырой токен в гит не попадал — коммит был откачен. Ротация на усмотрение.

**Не вопросы, а следующая работа движка** (никого не ждёт): P1-19 после Q18,
P1-31 после Q9, P1-32 после Q13, P1-22 после Q17, P2-1/P2-2 после Q10/Q11,
P2-8 после Q12; всё остальное в P2/P3 можно делать сразу.
